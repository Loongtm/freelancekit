...
(() => {
  ...
  /* Calculate totals with discount, shipping, VAT etc */
  function updateTotals(){
    ...
    // vat number presence could imply different tax logic (optional)
    const vat = subtotal * (parseNum(taxRate?.value)/100); // example
  ...
  function hydrate(data){
    try {
      sellerName && (sellerName.value = data.seller?.name||'');
      sellerEmail && (sellerEmail.value = data.seller?.email||'');
      sellerAddress && (sellerAddress.value = data.seller?.address||'');
      sellerPhone && (sellerPhone.value = data.seller?.phone||'');
      ...
      clientPhone && (clientPhone.value = data.client?.phone||'');
      invoiceNumber && (invoiceNumber.value = data.doc?.number||'');
      invoiceDate && (invoiceDate.value = data.doc?.date||'');
      dueDate && (dueDate.value = data.doc?.due||'');
      vatNumber && (vatNumber.value = data.doc?.vatNumber||'');
      discountRate && (discountRate.value = data.doc?.discountRate||'');
      shippingFee && (shippingFee.value = data.doc?.shippingFee||'');
      currency && (currency.value = data.doc?.currency||currency.value);
      taxRate && (taxRate.value = data.doc?.taxRate||taxRate.value);
      paymentMethod && (paymentMethod.value = data.doc?.paymentMethod||'');
      paidStatus && (paidStatus.checked = !!data.doc?.paid);
  ...
  function buildPrintHTML(autoPrint=false){
    ...
    return `<!doctype html><html lang="${I.getLang()}"><head><meta charset="utf-8"><title>${I.t('invoice')} ${d.doc.number}</title><link rel="icon" href="data:image/svg+xml;utf8,${LOGO_SVG}"><style>
      ...
      <header><div><img src="data:image/svg+xml;utf8,${LOGO_SVG}" alt=""><h2 style="display:inline-block;margin-left:8px">${I.t('invoice')}</h2></div><div>${d.doc.date}</div></header>
      <section class="section"><div><strong>${I.t('from')}</strong><div>${d.seller.name}</div><div>${d.seller.address}</div><div>${d.seller.email}</div><div>${d.seller.phone}</div></div>
      <div style="margin-top:8px"><strong>${I.t('billTo')}</strong><div>${d.client.name}</div><div>${d.client.address}</div><div>${d.client.email}</div><div>${d.client.phone}</div></div></section>
      <section class="section"><div><strong>${I.t('invoiceNumber')}</strong> ${d.doc.number}</div><div><strong>${I.t('dueDate')}</strong> ${d.doc.due}</div><div><strong>${I.t('vatNumber')}</strong> ${d.doc.vatNumber}</div></section>
      <table><thead><tr><th>${I.t('description')}</th><th>${I.t('qty')}</th><th>${I.t('unit')}</th><th>${I.t('taxPct')}</th><th class="r">${I.t('amount')}</th></tr></thead><tbody>${rowsHtml}</tbody><tfoot><tr><td colspan="4" class="r">${I.t('subtotal')}</td><td class="r">${subtotalEl?.textContent||''}</td></tr><tr><td colspan="4" class="r">${I.t('tax')}</td><td class="r">${taxEl?.textContent||''}</td></tr><tr><td colspan="4" class="r">${I.t('discount')}</td><td class="r">${d.doc.discountRate ? fmtMoney(parseNum(d.doc.discountRate)/100 * parseNum(subtotalEl.textContent.replace(/[^\d.-]/g,''))) : ''}</td></tr><tr><td colspan="4" class="r">${I.t('shippingFee')}</td><td class="r">${d.doc.shippingFee ? fmtMoney(parseNum(d.doc.shippingFee)) : ''}</td></tr><tr><td colspan="4" class="r">${I.t('total')}</td><td class="r">${totalEl?.textContent||''}</td></tr></tfoot></table>
      <footer><div><strong>${I.t('paymentMethod')}</strong><div>${d.doc.paymentMethod}</div></div><div style="margin-top:8px">${d.doc.paid?'<span style="color:green;"><strong>PAID</strong></span>':'<span style="color:red;"><strong>UNPAID</strong></span>'}</div><div style="margin-top:12px">Made with FreelanceKit — Contact: <a href="mailto:${I.t('contactEmail')}">${I.t('contactEmail')}</a></div></footer>
      ${script}</body></html>`;
  }

  function openHtml(html){ const blob = new Blob([html],{type:'text/html'}); const url = URL.createObjectURL(blob); const w = window.open(url,'_blank'); if(!w){ alert('Popup blocked'); URL.revokeObjectURL(url); return; } setTimeout(()=> URL.revokeObjectURL(url), 20000); }
