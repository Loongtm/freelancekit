/* FreelanceKit — Polished app.js (light by default) */
(() => {
  ...
  const setTheme=(v)=>{localStorage.setItem(THEME_KEY,v);document.documentElement.setAttribute('data-theme',v==='dark'?'dark':'light');};
  setTheme(getTheme());
  ...
  const langSel=byId('langSwitcher'); if(langSel){langSel.value=getLang(); langSel.addEventListener('change',()=>{setLang(langSel.value); applyI18n(); toast('Language updated');});}
  applyI18n();

  /* Toast + Net/PWA */
  const toasts=byId('toasts'); const toast=(m)=>{if(!toasts)return; const el=document.createElement('div'); el.className='toast'; el.textContent=m; toasts.appendChild(el); setTimeout(()=>el.remove(),2400);};
  ...
  $$('input,textarea,select').forEach(el=>{
    el.addEventListener('input',()=>{ markDirty(); /* 实时清除红框 */ setInvalid(el,!el.checkValidity()); });
    // 初次也根据默认值清一下
  ...
  /* Validate */
  const validateCore=()=>{let ok=true; [yourName,yourEmail,clientName,docType,docNo,docDate,currency].forEach(el=>{if(!el) return; const v=el.checkValidity(); setInvalid(el,!v); ok=ok&&v;}); if(!lineItemsWrap||lineItemsWrap.children.length===0){toast(t().plsAddItem); ok=false;} return ok;};
  ...
  /* Preview/Export (Blob URL) */
  const buildPrintHTML=(autoPrint=false)=>{const d=serialize(), lang=getLang(), tt=t(); const rowsHtml=$$('.item-row').map(row=>{const ds=$('.item-desc',row)?.value||''; const q=$('.item-qty',row)?.value||''; const p=$('.item-price',row)?.value||''; const tx=$('.item-tax',row)?.value||''; const amt=$('.item-amount',row)?.textContent||''; return `<tr><td>${ds}</td><td style="text-align:right">${q}</td><td style="text-align:right">${p}</td><td style="text-align:right">${tx}</td><td style="text-align:right">${amt}</td></tr>`;}).join(''); const script=autoPrint?`<script>addEventListener('load',()=>print(),{once:true})<\/script>`:''; return `<!doctype html><html lang="${lang}"><head><meta charset="utf-8"><title>${(d.doc?.type||'Document').toUpperCase()} ${d.doc?.no||''}</title><style>body{font:14px/1.6 system-ui,Segoe UI,Roboto,Arial;padding:28px;color:#111}h1{font-size:20px;margin:0 0 8px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:8px 0 16px}table{width:100%;border-collapse:collapse;margin-top:10px}th,td{border:1px solid #ddd;padding:6px}th{background:#f3f4f6;text-align:left}tfoot td{font-weight:bold}a{color:#0b57d0}</style></head><body><h1>${(d.doc?.type||'Document').toUpperCase()} ${d.doc?.no||''}</h1><div class="grid"><div><div style="color:#666">${tt.from}</div><div>${d.your?.name||''}</div><div>${d.your?.email||''}</div><div>${d.your?.address||''}</div><div>${d.your?.phone||''}</div></div><div><div style="color:#666">${tt.billTo}</div><div>${d.client?.name||''}</div><div>${d.client?.email||''}</div><div>${d.client?.address||''}</div><div>Ref: ${d.client?.ref||'-'}</div><div>Date: ${d.doc?.date||''} • Currency: ${d.doc?.currency||''}</div></div></div><table><thead><tr><th>${tt.description}</th><th style="text-align:right">${tt.qty}</th><th style="text-align:right">${tt.unit}</th><th style="text-align:right">${tt.tax}</th><th style="text-align:right">${tt.amount}</th></tr></thead><tbody>${rowsHtml}</tbody><tfoot><tr><td colspan="4" style="text-align:right">${tt.subtotal}</td><td style="text-align:right">${byId('subtotal')?.textContent||''}</td></tr><tr><td colspan="4" style="text-align:right">${tt.totalTax}</td><td style="text-align:right">${byId('taxAmount')?.textContent||''}</td></tr><tr><td colspan="4" style="text-align:right">${tt.total}</td><td style="text-align:right">${byId('grandTotal')?.textContent||''}</td></tr></tfoot></table><p><strong>${tt.notes}</strong><br>${byId('notes')?.value||'-'}</p><p><strong>${tt.terms}</strong><br>${byId('terms')?.value||'-'}</p><footer style="margin-top:40px;color:#666;font-size:12px"><hr><p>Made with FreelanceKit — For questions, contact: <a href="mailto:tmloong0128@gmail.com">tmloong0128@gmail.com</a></p></footer>${script}</body></html>`;};
  const openHtml=(html)=>{const blob=new Blob([html],{type:'text/html'}); const url=URL.createObjectURL(blob); const w=window.open(url,'_blank'); if(!w){toast('Popup blocked. Please allow popups.'); URL.revokeObjectURL(url); return;} setTimeout(()=>URL.revokeObjectURL(url),10000);};
  if(previewBtn) previewBtn.addEventListener('click',()=>{if(!validateCore()){toast(t().fixFields); return;} openHtml(buildPrintHTML(false));});
  if(exportBtn)  exportBtn.addEventListener('click',()=>{if(!validateCore()){toast(t().fixFields); return;} openHtml(buildPrintHTML(true)); toast(t().useDialog);});
